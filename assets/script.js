...
const pop = () => {
  ...
  $title.children[0].innerHTML = `Anjay, dapet<br>${prize.title}`;
  gsap.to($title, {
    delay: 1,
    y: "5vh",
    duration: 0.6,
  ...
  gsap.to($machine, {
    ...
    onComplete() {
      setTimeout(() => {
        gsap.to(".title-container .title", {
          opacity: 0,
          duration: 0.5,
          onComplete() {
            document.querySelector(".title-container .title").style.opacity = 1
            document.querySelector(".title-container .title .wiggle").innerText = ''
          }
        })
        gsap.to(".prize-container", {
          opacity: 0,
          duration: 0.5,
          onComplete() {
            init()
            prepare()
            document.querySelector(".prize-container").style.opacity = 1
          }
        })
        document.querySelector(".game-layer").classList.remove("dim")
      }, 3000);
...
const showHint2 = () => {
  $title.children[0].innerHTML = "Pencet buat klaim ngab!";
  gsap.set($pointer, {
    x: "16vh",
    y: "3vh",
  });
